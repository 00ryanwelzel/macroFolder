...
INSTANT = WaitTimes.instant
END_OF_ACTION = WaitTimes.end_of_action
...
def virtual_key_code(key: str) -> int:
    # Convert a key into a Windows keyboard APIs keycode.
...
# ---------------------------------
# --- Keyboard Sensor Functions ---
# ---------------------------------


def keydown(key: str) -> None:
    USER32.keybd_event(virtual_key_code(key), 0, KEYEVENTF_KEYDOWN, 0)


def keyup(key: str) -> None:
    USER32.keybd_event(virtual_key_code(key), 0, KEYEVENTF_KEYUP, 0)
...
def press_key(key: str) -> None:
    ...
    time.sleep(INSTANT)
    keyup(key)
    time.sleep(END_OF_ACTION)


def press_all_keys(hotkey: tuple[str, ...]) -> None:
    for key in hotkey:
        keydown(key)
...
def is_key_down(key: str) -> bool:
    return bool(USER32.GetAsyncKeyState(virtual_key_code(key)) & KEYEVENTF_KEYPRESSED)


def is_all_keys_down(hotkey: tuple[str, ...]) -> bool:
...
def moveto(destination: tuple[int, int], over: float | None = None) -> None:
...
def set_actionsleep(seconds: float) -> None:
    global END_OF_ACTION

    if seconds < 0:
        raise ValueError("ACTIONSLEEP cannot be negative")

    END_OF_ACTION = seconds


def get_actionsleep() -> float:
    return END_OF_ACTION
